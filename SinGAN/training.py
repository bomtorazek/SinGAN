...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,centers=None):
    ...
    z_opt = torch.full(fixed_noise.shape, 0, dtype = int, device=opt.device)
    z_opt = m_noise(z_opt)
    ...
    for epoch in range(opt.niter):
        ...
        for j in range(opt.Dsteps):
            ...
            if (j==0) & (epoch == 0):
                if (Gs == []) & (opt.mode != 'SR_train'):
                    prev = torch.full([1,opt.nc_z,opt.nzx,opt.nzy], 0, dtype = int, device=opt.device)
                    in_s = prev
                    prev = m_image(prev)
                    z_prev = torch.full([1,opt.nc_z,opt.nzx,opt.nzy], 0, dtype = int, device=opt.device)
...
def train_paint(opt,Gs,Zs,reals,NoiseAmp,centers,paint_inject_scale):
    in_s = torch.full(reals[0].shape, 0, dtype = int, device=opt.device)
    scale_num = 0
    nfc_prev = 0
